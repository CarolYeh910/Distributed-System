...
func startServer(hostAddr string, serviceType string, blockStoreAddrs []string) error {
	...
	if serviceType != "block" {
		meta_srv := surfstore.NewMetaStore(blockStoreAddrs)
		meta_srv.BlockStoreAddrs = blockStoreAddrs
		surfstore.RegisterMetaStoreServer(grpcServer, meta_srv)
	}
	if serviceType != "meta" {
		block_srv := surfstore.NewBlockStore()
		surfstore.RegisterBlockStoreServer(grpcServer, block_srv)
	}
